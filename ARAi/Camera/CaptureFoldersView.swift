        ...
        ZStack {
            ...
            List {
                ForEach(captureFolders, id: \.self) { folder in
                    CaptureFolderItem(model: model, url: folder, userData: userData)
                }
...
struct CaptureFolderItem: View {
    ...
    init(model: CameraViewModel, url: URL, userData: UserData) {
        self.model = model
    ...
    var body: some View {
        if ownedCaptureFolderState.captures.count > 1 {
        NavigationLink(destination: CaptureGalleryView(model: model,
                                                       observing: ownedCaptureFolderState, userData: userData)) {
            HStack {
