...
struct CaptureGalleryView: View {
    ...
    init(model: CameraViewModel, observing captureFolderState: CaptureFolderState, userData: UserData) {
        self.model = model
        self.userData = userData
    ...
    var body: some View {
        ZStack {
            ...
            NavigationLink(destination: CaptureFoldersView(model: model, userData: userData),
                           isActive: self.$showCaptureFolderView) {
                EmptyView()
            }
            .frame(width: 0, height: 0)
            .disabled(true)
//            .onChange(of: model.captureDir) { value in
//                captureFolderState.requestLoad()
//
//            }
            .onAppear() {
               
                //captureFolderState.captures = model.captureFolderState!.captures
              //  captureFolderState.requestLoad()
                refresh = true
...
//                        if let captures = model.captureFolderState?.captures {
//                            var capturesData = [Capture]()
//
//                            for capture in captures {
//                        if let captureDir = model.captureDir {
//                        sessionQueue.async {
//                            do {
...
//                                try capture.writeAllFiles(to: captureDir)
...
//                        }
                    ...
                    } .buttonStyle(CTAButtonStyle3())
                    
                } .padding()
                }
                Spacer()
                if model.captureDir == captureFolderState.captureDir {
                Button(action: {
//                    model.captureFolderState?.requestLoad()
                    //captureFolderState.requestLoad()
                      upload = true
